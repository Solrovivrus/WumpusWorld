...
class problemGenerator:
    ...
    def setPits(array, num):
        ...
        return array
    '''
    The setObs method first picks a random number to populate with blocked cells.
    No more than a quarter of the board will be filled with obstacles. Once number is picked, board
    is populated by assigning cells using a uniform probability distribution.
    '''
    @staticmethod
    def setObs(array, num):
        count = 0
        caveDim = np.shape(array)
        dim = caveDim[0] * caveDim[1]

        numObs = round(dim * float(num))
    ...
    @staticmethod
    def generateWorld(caves, wumpi, pits, blocks):
        ...
        # setting random number of obstacles in the cave
        for i in range(len(caves)):
            caves[i] = problem.setObs(caves[i], blocks)
            #print(caves[i])
